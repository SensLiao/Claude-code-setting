...
function main() {
  ...
  const estimate_high = num(o['estimate-high']);
  const actual_tokens = num(o.actual);
  if (estimate_high == null || actual_tokens == null) {
    console.error('budget-reconcile: bad args — require --estimate-high N and --actual N');
    process.exit(2);
  }
  const approved = num(o.approved);
