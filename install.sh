...
echo ""
warn "这个脚本只是无 node 环境的兜底 —— 它只复制文件。"
echo "  它不写 skillOverrides、不接线 hooks、不清 orphan、不写 pin。"
echo "  少了 skillOverrides，技能清单会停在平台默认：每个 skill 都抢描述预算，"
echo "  平台再按「调用最少优先丢」静默砍掉一批（本库实测 173 条里 112 条只剩裸名）。"
echo "  有 node 就改用完整路径："
echo "    node claude-config.js status         # 先看差异"
echo "    node claude-config.js update --apply # 覆盖 + skillOverrides + orphan + hooks"

if [[ $APPLY -eq 0 ]]; then
    echo ""
    warn "这是 DRY-RUN。仍要用本脚本部署请加 --apply："
    echo "    bash install.sh --apply"
    echo ""
    echo "  其它选项："
    echo "    bash install.sh --apply --force          # 覆盖已存在的文件（默认跳过已存在！）"
    echo "    bash install.sh --apply --target /path   # 自定义目标位置"
else
    echo ""
    info "下一步:"
    echo "  1. 启动 Claude Code，登录生成 ~/.claude/settings.json (含 OAuth)"
    echo "  2. 跑 node claude-config.js skills --apply 写入 skillOverrides（本脚本不做）"
    echo "  3. 跑 node claude-config.js wire --apply 接线 hooks（本脚本不做）"
    echo "  4. 在 ~/.claude.json 配置 MCP servers (见 mcp-configs/mcp-servers.json 参考)"
fi
