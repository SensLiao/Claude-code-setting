...
function isCodexPostToolUse(raw) {
  try {
    const input = raw.trim() ? JSON.parse(raw) : {};
    return String(input.hookEventName || '') === 'PostToolUse';
  } catch {
    return false;
  }
}

function writePassthrough(raw) {
  if (!isCodexPostToolUse(raw)) {
...
function emitHookResult(raw, output) {
  if (typeof output === 'string' || Buffer.isBuffer(output)) {
    ...
    if (!(isCodexPostToolUse(raw) && value === raw)) {
  ...
  if (output && typeof output === 'object') {
    ...
    if (Object.prototype.hasOwnProperty.call(output, 'stdout')) {
      const value = String(output.stdout ?? '');
      if (!(isCodexPostToolUse(raw) && value === raw)) {
        process.stdout.write(value);
      }
    } else if (!Number.isInteger(output.exitCode) || output.exitCode === 0) {
      writePassthrough(raw);
    }
...
function writeLegacySpawnOutput(raw, result) {
  ...
  if (stdout) {
    if (!(isCodexPostToolUse(raw) && stdout === raw)) {
