...
// Secret-shape patterns (v4.0.1 — H1B-004 remediation; force HIGH severity on any match).
// Anchored to reduce false positives (e.g. sk-modal CSS classes vs sk-[20+] OpenAI keys).
const SECRET_PATTERNS = [
  /-----BEGIN\s+(?:RSA\s+|EC\s+|OPENSSH\s+|DSA\s+)?PRIVATE\s+KEY-----/,
  /-----BEGIN\s+CERTIFICATE-----/,
  /aws_secret_access_key\s*=\s*[A-Za-z0-9/+=]{20,}/i,
  /aws_session_token\s*=\s*[A-Za-z0-9/+=]{20,}/i,
  /AKIA[A-Z0-9]{16}/,
  /ASIA[A-Z0-9]{16}/,
  /\bghp_[A-Za-z0-9]{30,}/,
  /\bgho_[A-Za-z0-9]{30,}/,
...
process.stdin.on('end', () => {
  ...
  try {
    ...
    // Secret-shape patterns (v4.0.1 — H1B-004 remediation; any match forces HIGH)
    for (const pattern of SECRET_PATTERNS) {
      if (pattern.test(content)) {
        findings.push('SECRET-SHAPE: ' + pattern.source.replace(/[()\\]/g, '').substring(0, 40));
        secretMatched = true;
      }
    }
    ...
    const severity = (secretMatched || findings.length >= 3) ? 'HIGH' : 'LOW';
