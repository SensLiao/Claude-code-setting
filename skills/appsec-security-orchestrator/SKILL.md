...
description: >-
  Application Security (AppSec) orchestrator for commercial web and server projects. Activate as soon as a project has backend / API / auth / user-data / file-upload / payment / admin / multi-tenant / GenAI-agent surface, or nears production — including when the ask names only one endpoint or one auth file. Small scope does not waive this route: a review done inline leaves no evidence trail, no ASVS / CSF mapping and no remediation routing. Maps NIST CSF 2.0 over a 6-layer capability map. Use for threat modeling, supply-chain / SCA / secret / SAST scanning, auth and input-validation and API security review, OWASP ASVS 5.0 / WSTG / API Top 10 mapping, headers and session review, remediation routing, DAST baseline planning. Casual triggers: "security review / is this secure / make sure it's secure / check for vulnerabilities / before we deploy / review the login API and auth handling / 安全审查 / 上线前检查". Performs no active scans — active validation is gated by `authorized-pentest-validation`.
trigger_phrases:
