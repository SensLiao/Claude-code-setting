...
| `i2r` / `idea-to-requirements-orchestrator` | `python ~/.claude/templates/i2r/scripts/i2r.py install --project .` | project-local toolchain (`i2r.py` + schemas + hooks) + hooks 注册进 `<project>/.claude/settings.json` (count: see `manifests/hook-registry.json`)。**I2R 变体**:命令是 `install`(不是 `init`——`init` 只 scaffold run folder);**无 config 文件**,hooks 以 `runs/i2r/` 存在与否 fail-open gate(不像其它 subsystem 用 `<sub>/config.json`)。 |
