...
standards_versions:
  ...
  - NIST CSF: 2.0 (ID.RA / ID.SC / PR.DS / PR.PS)
sensitive_data_rules:
  never_read: [".env*", "secrets/**", "*.pem", "*.key"]
  never_write: ["actual secret values", "real signing private keys", "cosign.key contents"]
  redact_on_output: ["tokens", "credentials", "registry passwords", "signing key material"]
upstream:
...
description: >
  ...
  malicious-package defenses, and build-pipeline integrity. Fills the
  orchestrator §5.2 `sbom_signing` gap and deepens `sca`. Maps to OWASP Top
  10:2025 A03, ASVS 5.0, SLSA, NIST SSDF / SP 800-161. Does NOT run active
  exploitation — wrapper/CLI scanning on the local repo only.
trigger_phrases:
...
3. **Reachability analysis**：漏洞函数是否真在调用路径上？不可达漏洞可降优先级（osv-scanner call-graph / 商业工具）。减噪是为了让有限修复预算打在真问题上——**不是**借口忽略。
...
Step 3  深度 SCA（Pillar 2）
        ...
        → transitive / lockfile integrity / reachability / VEX / fix-pin / license
...
- ❌ **不**在 chat / log / report 输出 raw signing key / registry 凭证——先走 `appsec-sdk redact`
...
7. 高危发现 → §9 schema finding → `appsec-sdk finding.add` → 路由 `security-remediation`
8. 供应链 evidence 经 `appsec-sdk evidence.append <tag> sca` 落盘，写入 **AppSec Release Evidence §9 Platform Layer**（**不是** SECURITY.md §13 — §13 仅含 Security Headers）
9. 给 `risk-register` 补条目（每个高危）

### Finding schema 对齐（orchestrator §9）

- `source`：用现有 enum——依赖漏洞用 `sca`；镜像层用 `container_scan`；不发明新值（PreToolUse prewrite hook 拒 schema 漂移）。
...
- `asvs_mapping[]`：versioned `v5.0.0-<ch>.<sec>.<req>` 三段格式（如 `v5.0.0-13.4.1`、`v5.0.0-15.1.1`）；regex `^v5\.0\.0-\d+\.\d+\.\d+$`。
- `severity` / `confidence`：**小写** enum（`critical|high|medium|low` / `high|medium|low`）。
- `owasp_top10[]`：供应链类用 `A03:2025`（Software Supply Chain Failures）；完整性类可加 `A08:2025`。
- `csf_function`：多为 `ID`（识别/SCA 清点）或 `PR`（保护/签名验证）。
- `detector`：工具名 + 版本（如 `syft@1.x` / `osv-scanner@1.x` / `cosign@2.x`）。
...
- [security-platform-iac-cloud](../security-platform-iac-cloud/SKILL.md) (Dockerfile lint / 镜像 CVE / K8s)
