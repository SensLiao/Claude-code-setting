...
name: image-to-code
description: >-
  Website image-to-code skill for Codex — for visually important web tasks it must first generate the design image(s), analyze them deeply, then implement the site to match. Prefers large, readable, section-specific images over tiny compressed boards, generates fresh standalone images per section instead of cropping old ones, avoids under-generation and cards-inside-cards UI, and keeps the hero clean, spacious and visible on a small laptop.
---
...
```
1. read user-provided PNG(s)            ← do NOT regenerate
2. deep analysis (use §9 standard)
3. promote into design/anchor/pages/    (after user review)
4. emit HTML/JSX layout skeleton into design/prototype/<phase>/
5. user iterates on skeleton via prompt refinement (NOT image regeneration)
```

mode=reference does not override §3 (generate enough images) for the user's images — it overrides §2's "we generate first" default.
