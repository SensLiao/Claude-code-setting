...
- **workflow-spec mode（parent §18 / qa-orchestrator.js）— 预备 / 未接线（RESERVED, NOT WIRED）**：
  - parent §17.1 列出的 dedicated runner **`qa-contract-runner` 明确标注「预备」**，与其余 5 个 R2 runner（static/component/visual/a11y/perf）不同——后者已 wiring-audited，`qa-contract-runner` **尚未接入** `qa-orchestrator.js` 的 deterministic spec phases。
  - 直接后果：**在 workflow-spec 运行（含 `/qa-release-readiness` / `/qa-commercial-cert`）里，contract 层目前不产出 evidence**——它不在任何 preset 的 phases 里被 fan-out，`.qa/evidence/<tag>/contract.yaml` 不会被该模式自动生成。
  - 不要伪装：在 workflow-spec gate 报告里，contract 层应显式标 `NOT_WIRED — contract layer reserved in workflow-spec mode; run prompt-only dispatch or wait for qa-contract-runner wiring`，**不得**记为 PASS、也不得静默从 layers_required 抹掉。若该 release 实际触及 API boundary（§2 触发条件成立），正确做法是**在 prompt-only 路径补跑本 skill** 取证据，或显式登记为 residual risk + owner（parent §11 / `qa-evidence-bundle`）。

- **后续 wire 需要做的（交主控 / 后续 slice，不在本 slice 范围）**：① 创建 / 启用 `~/.claude/agents/qa-contract-runner.md`（agentType frontmatter + 输出 `contract_testing` schema 或 workflow-spec 对应 `CONTRACT_SCHEMA.v1`）；② 在 `qa-orchestrator.js` 相关 preset 的 phases 里加 contract phase（fan-out by changed API surfaces）；③ parent §17.1 把「预备」改为「runtime-PROVEN」前需 cold-start wiring audit。本 slice **仅文档标注 gap，不动 runner / orchestrator / preset / schema**。
