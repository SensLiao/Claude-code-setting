...
**3 orchestration agents + 6 dedicated runners = 9**：orchestration = `qa-risk-classifier`(opus) · `qa-evidence-validator`(sonnet) · `qa-flaky-triager`(sonnet)（即 §1.6.2 binding 三件套）；runners（B.1.f R2 dedicated）= `qa-static-baseline-runner` · `qa-component-runner` · `qa-contract-runner`(预备) · `qa-visual-runner` · `qa-a11y-runner` · `qa-perf-runner`。每个必须输出 §16 schema YAML（或 workflow-spec 对应 `*_SCHEMA.v1`），否则视为 dispatch 失败。
