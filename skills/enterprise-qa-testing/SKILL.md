...
| Mode | 用途 | Step 6/7 stdout 要求 | Release Readiness 输出 |
|---|---|---|---|
| `execution` | 真实 CI 或本地完整执行 | **必须**附实际 stdout / artifact | pass / fail / block |
| `plan-only` | 已有项目但 CI 未跑（PR 准备阶段、本地策略评估） | 可标 `BLOCKED — plan-only, see §1.5` | conditional-pass plan / blocked-pending-evidence |
| `design-only` | 设计阶段、无运行环境、纯方案评估 | 整个 Step 6/7 标 `BLOCKED — design-only` | strategy / not yet releasable / preconditions list |
...
### 1.6.2 Agent Binding（3 个 named agents 必须 present）

本 skill 强依赖以下 user-level agents：
...
#### A. QA-owned Child Skills（13 个，本 skill 直接 dispatch）
...
### Step 5. 文件改动清单
- `__tests__/xxx.test.ts` — 新增
- `playwright/xxx.spec.ts` — 新增

### Step 6. Fast Lane 运行结果
**Mode**: execution / plan-only / design-only

若 mode = `execution`：
\`\`\`bash
npx vitest run --reporter=verbose
# [实际 stdout，包含 exit code]
\`\`\`

若 mode = `plan-only` 或 `design-only`：
\`\`\`
BLOCKED — <mode>, see §1.5
预计执行命令：npx vitest run --reporter=verbose
预计 unblock 条件：[列出，如 "需要 npm ci 完成 + DB 可用 + .env 配置"]
\`\`\`

### Step 7. Deep Layers 运行结果
**Mode**: execution / plan-only / design-only

若 mode = `execution`：
\`\`\`bash
npx playwright test --reporter=html
# [实际 stdout + artifact 路径]
\`\`\`
...
BLOCKED — <mode>, see §1.5
预计执行命令：[逐层列出]
预计 artifact：[playwright-report/, lighthouse-report.html, a11y-junit.xml]
预计 unblock 条件：[列出]
\`\`\`
...
9 个：`qa-risk-classifier`(opus) · `qa-evidence-validator`(sonnet) · `qa-flaky-triager`(sonnet) · `qa-static-baseline-runner` · `qa-component-runner` · `qa-contract-runner`(预备) · `qa-visual-runner` · `qa-a11y-runner` · `qa-perf-runner`（后 6 = B.1.f R2 dedicated runners）。每个必须输出 §16 schema YAML（或 workflow-spec 对应 `*_SCHEMA.v1`），否则视为 dispatch 失败。
...
11. 等 reply，match approval whitelist（精确，大小写无关，trim）：EN `OK/okay/approve/approved/go/yes/proceed/ship it/LGTM`；CN `跑/批准/同意/继续/好/执行`。commercial-cert 额外要求 reply 含数字（批准的 estimate_high）OR 复述 banner。不 match → 不写 sentinel → 不 launch
