...
## Agent + Model 路由(2026-07-29 从 rules/common/agents.md 迁入)

> 上面的 Layer 段落回答"哪层用哪个 skill";本段回答"派哪个 agent、用哪个 model"。
> 迁入原因:原先常驻在 `rules/common/agents.md`,但只有真跑 UI 时才用得上 —— 移到本 reference 后随 orchestrator 按需加载。完整 skill 列表见 `~/.claude/SKILLS-INDEX.md`。
