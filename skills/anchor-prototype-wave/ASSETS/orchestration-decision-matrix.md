...
| **Subagent quota exhausted** | Spawn returns error before subagent starts | medium | Pause the wave. Wait for quota window. Resume with remaining subagents. Consider falling back: if Claude subagent quota is exhausted, delegate execution agents to the Codex official plugin (`/codex:rescue`, orchestrator stays as main thread). |
