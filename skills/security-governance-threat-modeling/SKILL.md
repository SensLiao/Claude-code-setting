...
Step 10 输出
        → 一份完整 threat-model-STRIDE.md（per system / feature）
...
3. `risk-register.md` 更新（每个 STRIDE OPEN gap **和** R&C OPEN gap → 一条 Risk Register entry，同等优先级处理）
4. Threat Model 摘要回写到 `SECURITY.md` §1-3
5. Trigger conditions for re-model 写明
6. 下次 review 日期
...
- [templates/threat-model-STRIDE.md](../appsec-security-orchestrator/templates/threat-model-STRIDE.md) — 模板
