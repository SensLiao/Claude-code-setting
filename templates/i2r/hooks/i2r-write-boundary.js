...
// not do GSD's job. Allowed: .i2r, docs/requirements, docs/adr, the skill dir.
...
// happens to contain a denied substring (e.g. a raw mirror .i2r/.../raw/src/, docs/requirements/api/).
const allowed = ['/.i2r/', '/docs/requirements/', '/docs/adr/', '/.claude/skills/idea-to-requirements-orchestrator/'];
...
if (denied.some(d => fp.includes(d))) {
  process.stderr.write('[I2R write-boundary] BLOCKED: write to an implementation path while I2R is active:\n  '
    + fp + '\nI2R produces requirements only (WHAT/WHY). Implementation belongs to downstream orchestration.');
  process.exit(2);
}
process.exit(0);
